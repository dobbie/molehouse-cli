...
show_uninstall_help() {
    ...
    echo "  mo uninstall --list --json     Same, as a machine-readable JSON envelope"
    echo "  mo uninstall --plan slack      Preview every file removing Slack would delete (JSON)"
    echo "  mo uninstall --apply-plan      Apply a plan read back on stdin with a selection"
    echo ""
    echo "Options:"
    echo "  --list            List installed apps with the exact name mo uninstall accepts"
    echo "  --json            With --list, emit a JSON envelope instead of a table"
    echo "  --plan NAME       Non-interactive per-file removal preview for one app (JSON)"
    echo "  --apply-plan      Read a plan plus a \"selected\" id array on stdin and remove"
    echo "                    only those entries; refuses if the plan no longer matches disk"
    echo "  --dry-run         Preview app uninstallation without making changes"
    echo "  --permanent       Bypass macOS Trash and rm -rf immediately; with --plan it"
    echo "                    records the semantics the plan's verdicts assume"
