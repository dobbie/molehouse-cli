...
# Field 7 is requires_sudo (Molehouse CONTRACT.md §8.4, M1-T3 Gap 2):
# evidence-based, true only for a handler (or a direct helper) with a real
# sudo/privileged-launchctl call path. See docs/handoff-M1-T3.md for the
# per-task source evidence this column was populated from.
expected=$(cat <<'CONTRACT'
system_maintenance|opt_system_maintenance|DNS & Spotlight Check|DNS & Spotlight Check|Refresh DNS cache & verify Spotlight status|true|true
cache_refresh|opt_cache_refresh|Finder Cache Refresh|Finder Cache Refresh|Refresh QuickLook thumbnails & icon services cache|true|false
saved_state_cleanup|opt_saved_state_cleanup|App State Cleanup|App State Cleanup|Remove old saved application states (30+ days)|true|false
fix_broken_configs|opt_fix_broken_configs|Broken Config Repair|Broken Config Repair|Fix corrupted preferences files|true|false
network_optimization|opt_network_optimization|Network Cache Refresh|Network Cache Refresh|Optimize DNS cache & restart mDNSResponder|true|true
sqlite_vacuum|opt_sqlite_vacuum|Database Optimization|Database Optimization|Compress SQLite databases for Mail, Safari & Messages (skips if apps are running)|true|false
launch_services_rebuild|opt_launch_services_rebuild|LaunchServices Repair|LaunchServices Repair|Repair "Open with" menu & file associations|true|false
prevent_network_dsstore|opt_prevent_network_dsstore|Prevent Finder .DS_Store|Prevent Finder .DS_Store|Set a persistent Finder preference to stop writing .DS_Store on SMB/AFP/NFS and USB volumes|true|false
legacy_overrides_audit|opt_legacy_overrides_audit|Legacy Overrides|Legacy Overrides|Remove hidden App Nap and disk-image verification overrides left by old tweak tools|true|false
network_stack_optimize|opt_network_stack_optimize|Network Stack Refresh|Network Stack Refresh|Flush routing table and ARP cache to resolve network issues|true|true
disk_permissions_repair|opt_disk_permissions_repair|Permission Repair|Permission Repair|Fix user directory permission issues|true|true
spotlight_index_optimize|opt_spotlight_index_optimize|Spotlight Optimization|Spotlight Optimization|Rebuild index if search is slow (smart detection)|true|true
spotlight_orphan_rules_cleanup|opt_prune_spotlight_orphan_rules|Spotlight Orphan Rules|Spotlight Orphan Rules|Remove Spotlight search-rule entries for apps that are no longer installed|true|false
...
coreduet_cleanup|opt_coreduet_cleanup|Usage Data|Usage Data|Clean old usage tracking data|true|false
CONTRACT
)
...
@test "optimize catalog resolves requires_sudo by exact action id" {
    run env PROJECT_ROOT="$PROJECT_ROOT" /bin/bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/catalog.sh"

[[ "$(optimize_catalog_requires_sudo_for system_maintenance)" == "true" ]] || exit 1
[[ "$(optimize_catalog_requires_sudo_for cache_refresh)" == "false" ]] || exit 1
if optimize_catalog_requires_sudo_for unknown_action; then
...
if /bin/bash --noprofile --norc < <(
    awk '
        !changed && match($0, / true (true|false)$/) {
            req = (substr($0, RSTART, RLENGTH) == " true true") ? "true" : "false"
            print substr($0, 1, RSTART - 1) " false " req
            changed = 1
            next
        }
        { print }
